namespace $.$$ {
	...
	export class $bog_formigo_filler_step extends $.$bog_formigo_filler_step {
		...
		@ $mol_mem
		override label() {
			return this.field()?.Label()?.val() ?? ''
		}

		@ $mol_mem
		override hint() {
			return this.field()?.Hint()?.val() ?? ''
		}

		@ $mol_mem
		override required_text() {
			return this.field()?.Required()?.val() ? 'Required' : ''
		}

		@ $mol_mem
		field_type() {
			return this.field()?.Type()?.val() ?? 'str'
		}

		@ $mol_mem
		override enum_options() {
			const opts = this.field()?.Options()?.items() ?? []
		...
		input_widget() {
			switch (this.field_type()) {
				case 'text': return this.Textarea_input()
				case 'int':
				case 'real': return this.Number_input()
				case 'bool': return this.Check_input()
				case 'enum': return this.Select_input()
				case 'date': return this.Date_input()
				case 'file': return this.File_input()
				default: return this.String_input()
			}
		}
